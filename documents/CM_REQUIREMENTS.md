...
| 2.4 | 2026-06-12 | **전송 후 잔류 톡방 일괄 정리** `kakao_send.close_rooms()` — 이미지 방(업로드 보호로 esc 정리 생략)·오류 중단 방을 전체 전송 완료 후 제목 매칭(`EnumWindows`, 공백 무시 정규화)으로 `WM_CLOSE` 일괄 발송. 키 입력이 아닌 창 메시지라 포커스 불필요. 2초 유예 후 실행, 업로드 진행 중인 창은 카톡이 닫기 보류 → 자동 확인 없이 유지(업로드 보호), 상태바에 "N개 정리 · M개 업로드 중 유지" 보고. 비 Windows no-op |
