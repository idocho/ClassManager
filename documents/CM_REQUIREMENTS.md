...
| 2.4 | 2026-06-12 | **전송 후 잔류 톡방 일괄 정리** `kakao_send.close_rooms()` — 이미지 방(업로드 보호로 esc 정리 생략)·오류 중단 방을 전체 전송 완료 후 `WM_CLOSE` 일괄 발송(키 입력 아닌 창 메시지 — 포커스 불필요). **오폐쇄 2중 가드**: ① 카카오톡 프로세스(PID) 소속 창만 — 학생 이름이 제목에 든 타 앱 오인 차단 ② 제목 전방일치(방이름+인원수 suffix만, 포함 비교 불가 — 파괴 동작이므로 읽기 검증보다 엄격). **기존 보호 불변**: WM_CLOSE는 사용자 X 클릭과 동일 경로 — 업로드 중이면 카톡 확인 팝업이 닫기를 보류하고 코드는 그 팝업을 절대 자동 조작하지 않음(확인=업로드 취소). 2초 유예 후 실행, 상태바 "N개 정리 · M개 업로드 중 유지". 비 Windows no-op |
